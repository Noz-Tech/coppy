...
private fun templateFieldsFor(type: EntryType): List<CustomFieldUi> = when (type) {
    ...
    EntryType.BankAccount -> listOf(
        ...
        CustomFieldUi(label = "Branch Code"),
        ...
        Scaffold(
            ...
            Column(
                ...
                Column(
                    ...
                    if (entryType == EntryType.Card) {
                    ...
                    } else if (entryType == EntryType.BankAccount) {
                        val bankAccountRowFields = setOf("Branch Code", "SWIFT / IBAN")
                        customFields
                            .filterNot { it.label in bankAccountRowFields }
                            .forEach { field ->
                                key(entryType.value, field.label) {
                                    EntryFieldTextField(
                                        field = field,
                        ...
                        val routingField = customFields.firstOrNull {
                            it.label == "Branch Code"
                        }
                        val swiftField = customFields.firstOrNull { it.label == "SWIFT / IBAN" }
                        if (routingField != null && swiftField != null) {
                            BoxWithConstraints(modifier = Modifier.fillMaxWidth()) {
                                val fieldWidth = (maxWidth - 10.dp) / 2
                                Row(
                                    horizontalArrangement = Arrangement.spacedBy(10.dp),
                                    ...
                                    key(entryType.value, routingField.label) {
                                        EntryFieldTextField(
                                            field = routingField,
                                            onValueChange = { input ->
                                                customFields = customFields.map {
                                                    if (it.label == routingField.label) {
                                                        it.copy(value = input)
                                                    } else it
                                                }
                                            },
                                    ...
                                    key(entryType.value, swiftField.label) {
                                        EntryFieldTextField(
                                            field = swiftField,
                                            onValueChange = { input ->
                                                customFields = customFields.map {
                                                    if (it.label == swiftField.label) {
                                                        it.copy(value = input)
                                                    } else it
                                                }
                                            },
